...
public class Income {
    public Main main;
    private ArrayList<TransactionList> salaryList;

    public Income(Main main, Transaction transaction) {
        this.main = main;
    ...
    public void main(String[] args) {
        ...
        while (isIncome) {
            System.out.println("Choose to view your deposite:");
    ...
    /*
     * Beroende på användarens input så skrivs ut summan antingen per år, år-vecak, år-månad,
     * år-månad-dag eller den totala summan.
     */
    public void printYearlyTransactions(int year) {
        double sum = 0;
        System.out.println("Transaction for the year " + year + ":");
    ...
    public void printMonthlyTransactions(int year, int month) {
        double sum = 0;
        System.out.println("Transaction for the " + year + "-" + month + ":");
        for (TransactionList entry : salaryList) {
    ...
    public void printWeeklyTransactions(int year, int week) {
        ...
        System.out.println("Transactions for year " + year + ", week " + week);
        for (TransactionList entry : salaryList) {
            LocalDate date = entry.getDate();
    ...
    public void printDailyTransactions(int year, int month, int day) {
        double sum = 0;
        System.out.println("Transactions for " + year + "-" + month + "-" + day);
        for (TransactionList entry : salaryList) {
            LocalDate date = entry.getDate();
            if (date.getYear() == year && date.getMonthValue() == month
    ...
    public void printOverallTransactions() {
        ...
        System.out.println("Deposite sorted by date: ");
        for (TransactionList entry : salaryList) {
            System.out.println(entry);
            sum += entry.getAmount();
        }
        System.out.println("Your current Deposite is: " + sum);
    }
}
